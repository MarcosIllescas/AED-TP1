...
\pred{caminoValido}{\In t: \TLista{\ent}, A: \TLista{\TLista{\ent}}}{
|t| \geq 2 \wedge (\forall i: \ent)(0 \leq i < |t| \implicaLuego 0 \leq t[i] < |A|)
}

\pred{matrizCuadrada}{\In s: \TLista{\TLista{\ent}}}{
(\forall i: \ent)(0 \leq i < |s| \implicaLuego |s| = |s[i]|)
}

\pred{matrizPositivaSimetricaConCerosDiagonal}{\In s: \TLista{\TLista{\ent}}}{
(\forall i: \ent)(\forall j: \ent)(0 \leq i < j < |s| \implicaLuego s[i]_{i} = 0 \wedge  s[i]_{j} = s[j]_{i} \wedge s[i]_{j} \geq 0)
}

\pred{esCamino}{\In s: \TLista{\ent}, \In n: \ent, \In m: \ent, \In A: \TLista{\TLista{\ent}}}{
|s| \geq 2 \wedge s[0] = n \wedge s[|s|-1] = m \wedge \\(\forall i: \ent)(0 \leq i < |s| \implicaLuego 0 \leq s[i] < |A|) \wedge \\ (\forall j: \ent)(0 \leq j < |s| - 1 \implicaLuego A[s[j]]_{s[j+1]} > 0)
}
...
\begin{proc}{hayCamino}{\In distancias : \TLista{\TLista{\ent}}, \In desde : \ent, \In hasta : \ent}{\bool}
	\requiere{matrizCuadrada(distancias) \wedge matrizPositivaSimetricaConCerosDiagonal(distancias) \wedge \\ 0 \leq desde < |distancias| \land 0 \leq hasta < |distancias| \wedge desde \neq hasta}
...
\begin{proc}{cantidadCaminosNSaltos}{\Inout conexion: \TLista{\TLista{\ent}}, \In n: \ent}{}
	\requiere{n \geq 1 \wedge matrizCuadrada(conexion) \wedge \\ matrizPositivaSimetricaConCerosDiagonal(conexion) \wedge unosYceros(conexion) \wedge conexion = C_0}
\asegura{(\exists s: \TLista{\TLista{\TLista{\ent}}})(|s| = n \wedge  s[0] = C_0 \wedge \\
(\forall i: \ent)(0 \leq i < n \implicaLuego matrizCuadrada(s[i]) \wedge matrizPositivaSimetricaConCerosDiagonal(s[i])) \wedge \\
(\forall j: \ent)(1 \leq j < n \implicaLuego esMultiplicacionMatricialDe(s[j], s[j-1], C_0)) \wedge conexion = s[n-1])}
\end{proc}

\pred{unosYceros}{\In s: \TLista{\TLista{\ent}}}{
(\forall i: \ent)(\forall j: \ent)(0 \leq i < j < |s| \implicaLuego s[i]_{j} = 0 \vee s[i]_{j} = 1)
}

\pred{esMultiplicacionMatricialDe}{\In A: \TLista{\TLista{\ent}}, \In B: \TLista{\TLista{\ent}}, \In C: \TLista{\TLista{\ent}}}{
(\forall i: \ent)(\forall j: \ent)(0 \leq i \leq j < |A| \implicaLuego A[i]_{j} = \sum_{k=0}^{|A|-1}{B[i]_{k}*C[k]_{j}})
}

\subsection{caminoMínimo}

\begin{proc}{caminoMinimo}{\In origen: \ent, \In destino: \ent, \In distancias: \TLista{\TLista{\ent}}}{\TLista{\ent}}
	\requiere{matrizCuadrada(distancias) \wedge matrizPositivaSimetricaConCerosDiagonal(distancias) \wedge \\ 0 \leq origen < |distancias| \land 0 \leq destino < |distancias| \wedge origen \neq destino}
	\asegura{(|res| = 0 \longleftrightarrow \neg(\exists s: \TLista{\ent})(esCamino(s, origen, hasta, distancias))) \vee \\ (esCamino(res, origen, hasta, distancias) \wedge (\forall c: \TLista{\ent})(esCamino(c, origen, destino, distancias) \longrightarrow \\
 distanciaTotal(res, distancias) \leq distanciaTotal(c, distancias)))}
    \aux{distanciaTotal}{\In s: \TLista{\ent}, \In A: \TLista{\TLista{\ent}}}{\ent}{\sum_{i=0}^{|c|-2} A[c[i]]_{c[i+1]}}
\end{proc}
