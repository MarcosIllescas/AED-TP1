...
\pred{mismosNombres}{\In s: \TLista{Ciudades}, \In t: \TLista{Ciudades}}{(\forall i: \ent) (0 \leq i < |s| \implicaLuego (\exists j: \ent) (0 \leq j < |t| \yLuego s[i].nombre = t[j].nombre))}

\pred{siCoincidenNombresSumoHabitantes}{\In s: \TLista{Ciudades}, \In t: \TLista{Ciudades}, \In r: \TLista{Ciudades}, \In i: \ent, \In j: \ent, \In k: \ent}{(s[i].nombre = t[j].nombre \; \wedge \;  s[i].nombre = r[k].nombre) \longrightarrow  s[i].habitantes = t[j].habitantes + r[k].habitantes}
